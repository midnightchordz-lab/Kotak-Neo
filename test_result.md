...
test_plan:
  ...
  - task: "Live Options Price Fetching (Kotak API)"
    implemented: true
    working: "NA"
    file: "/app/backend/kotak_options_service.py"
    stuck_count: 1
    priority: "high"
    ...
    status_history:
      - working: false
        agent: "user"
        comment: "400 Bad Request error 'Invalid neosymbol values' when fetching live option prices. API receives text-based trading symbol instead of correct format."
      - working: "NA"
        agent: "main"
        comment: "Fixed kotak_options_service.py to use trading_symbol (e.g., NIFTY25MAR23000CE) for Kotak Quotes API instead of numeric instrument token. Updated get_batch_quotes() and build_options_chain() functions. Requires authentication to test live data."
